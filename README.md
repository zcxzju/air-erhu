...
- **女生 3D 演奏场景**：使用 Three.js 加载 `assets/erhu-player.glb`，展示人物、二胡和弓的完整演奏姿态。
...
- 模型资源：`assets/erhu-player.glb`
- 加载示意图：`assets/erhu-player.png`
- 声音：原生 Web Audio API
- 旧版页面：`classic.html`

当前提供的 GLB 是静态模型，页面已加入随弓运动的整体姿态联动；如果要让手指、手臂逐关节真实弯曲，需要替换为带骨骼和动画的 `.glb/.gltf` 模型。
