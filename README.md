...
- **左侧 · 琴杆（按弦区）**：竖直琴杆上排布 D 调第一把位音位（内弦 1 2 3 4 5、外弦 5 6 7 i 2'），点按即按弦。
- **右侧 · 弓区**：横向琴弓跟随鼠标左右滑动，移动越快声音越亮越大，像真的拉弓。
- **内置示范**：《菊花台》旋律示范（约 32 秒），播放时弓法与指法自动演示。
...
| 键盘 1–9 | 选音（1–5 内弦，6–9 外弦 6 7 i 2'） |
| 键盘 ← / → | 模拟拉弓 |
| 右上角静音开关 | 静音，偏好记忆在本地 |
...
- 二胡音色合成：锯齿波 + 三角波叠层、低通滤波随弓速开合、5.5Hz 揉弦（LFO 调制音高）、弓毛噪声（带通滤波白噪声）、换音 60ms 滑音。
- 示范曲以 `audioContext.currentTime` 为唯一时钟调度，音画同步；弓法按短语交替推弓/拉弓，长音拆弓。
- 二胡定弦 D4–A4（D 调，1-5 弦）：内弦空弦 = 1，外弦空弦 = 5。

## 示范曲旋律来源

《菊花台》（周杰伦 曲）公开编配简谱（C 调）的旋律主干，转写至 D 调二胡把位，含间奏、主歌、副歌、尾声四段。仅作学习与演示用途，版权归原权利人所有。
