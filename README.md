...
- **左侧 · 琴杆（按弦区）**：竖直琴杆上排布当前调的七声音位，点按即按弦；琴杆上有随音位上下移动的左手按弦示意，当前按弦手指会高亮。
- **六个定调**：支持 `1=G`、`1=F`、`1=D`、`1=C`、`1=B♭`、`1=A`；换调会同步更新琴杆数字、数字键 1–7 的音高和《菊花台》示范。
- **乐器视觉**：参考传统二胡的两根弦、木质六棱琴筒、蟒皮面、琴码、琴轴与尾柱，并参考真实演奏姿态，在 Canvas 中重画了带木纹、蟒皮鳞格、边缘高光、体积阴影、手腕和掌面关节细节的交互模型；未直接打包第三方 3D 文件。
...
| 点按琴杆上的音位 | 按弦换音；选中空弦时左手不按弦（初始为空弦内弦 1） |
